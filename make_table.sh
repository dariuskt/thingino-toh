...
function addCam() {
	...
	sd_card=$(cat ${cam_file} | fgrep -q 'BR2_THINGINO_SDCARD=y' && echo true || echo false)
	pan_tilt=$(cat ${cam_file} | fgrep -q 'BR2_THINGINO_MOTORS=y' && echo true || echo false)
	mic=$(cat ${cam_file} | fgrep -q 'BR2_THINGINO_AUDIO=y' && echo true || echo false)
